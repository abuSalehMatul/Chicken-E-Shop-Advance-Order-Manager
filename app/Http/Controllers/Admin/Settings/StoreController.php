...
class StoreController extends Controller{
	...
	function cities_list(Request $request, $id){
		if(!empty($request->session()->has('id') && $request->session()->get('role') == 0 && $id)){
	        ...
	        $html = '';
	        if(count($result) > 0){
	        	$html .= '<label class="label-control">City</label><select id="city" name="city" class="form-control" style="width: 100%"><option>No City Selected</option>';
	        	foreach($result as $row){
	        	...
	        	$html .= '<label class="label-control">City</label><select id="city" name="city" class="form-control" style="width: 100%"><option>No City Found</option></select>';
	            $ajax_response_data['ERROR'] = 'TRUE';
	            $ajax_response_data['DATA'] = $html;
	            echo json_encode($ajax_response_data);
	        }
    ...
    function edit(Request $request){
    	if(!empty($request->session()->has('id') && $request->session()->get('role') == 0)){
			...
			if(!empty($result['query'])){
         	...
         	//Query For Getting Store Map
	 		$query = DB::table('tbl_store_map')
	 		             ->select('*')
	 		             ->where('user_id', $request->session()->get('id'));
         	$result['store_map'] = $query->first();
    	...
    	if(!empty($request->session()->has('id') && $request->session()->get('role') == 0)){
    		if($request->input('btn') == 0){
		        ...
		        $input_validations = $request->validate([
		            ...
		            'zip' => 'nullable',
		            'code_1' => 'nullable',
		            'phone_1' => 'nullable|mix:10|max:10|unique:tbl_store_settings,user_id,'.$request->session()->get('id'),
		            'code_2' => 'nullable',
		            'phone_2' => 'nullable|mix:10|max:10|unique:tbl_store_settings,user_id,'.$request->session()->get('id'),
	        	...
	        	$user_id = $request->session()->get('id');
	        	$ip_address = $request->ip();
	            $header_image = $request->file('header_image', 'unammed');
	            $footer_image = $request->file('footer_image', 'unammed');
	            $favicon_image = $request->file('favicon_image', 'unammed');
				$created_date = date('Y-m-d');
	            $created_time = date('H:i:s');
		        ...
		        if(empty($header_image) && empty($footer_image) && empty($favicon_image)){
		        	//Flash Error Message
