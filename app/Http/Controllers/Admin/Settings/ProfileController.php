	...
	function edit(Request $request){
    	if(!empty($request->session()->has('id')) && $request->session()->get('role') == 0){
	    	...
	    	$query = DB::table('users')
	    	             ->select('users.id as u_id', 'first_name', 'last_name', 'cnic', 'date_of_birth', 'gender', 'country_id', 'city_id', 'address', 'country_code_1', 'cell_number1', 'country_code_2', 'cell_number2', 'password', 'image', 'users_social_profile.id as u_s_id', 'user_id', 'facebook', 'twitter', 'googleplus')
	    	             ->leftJoin('users_social_profile', 'users_social_profile.user_id', '=', 'users.id')
    	...
    	if(!empty($request->session()->has('id')) && $request->session()->get('role') == 0){
    		if($request->input('btn') == 0){
		        ...
		        $input_validations = $request->validate([
		            'first_name' => 'nullable',
		            'last_name' => 'nullable',
		            'cnic' => 'nullable|regex:/^[0-9]{13}$/|unique:users',
		            'dob' => 'nullable',
		            'gender' => 'nullable',
		            'country' => 'nullable',
		            'city' => 'nullable',
		        ...
		        $input_validations = $request->validate([
		        	'code_1' => 'nullable',
		            'cell_number1' => 'nullable|regex:/^[0-9]{10}$/|unique:users',
		            'code_2' => 'nullable',
        			'cell_number2' => 'nullable|regex:/^[0-9]{10}$/|unique:users',
		        ]);
	            ...
	            $created_time = date('H:i:s');
	     		...
	     		//Inputs Validation
		        $input_validations = $request->validate([
		        	'facebook' => 'nullable|unique:users_social_profile',
		        	'twitter' => 'nullable|unique:users_social_profile',
		        	'googleplus' => 'nullable|unique:users_social_profile',
	        	]);
		        ...
		        //Query For Updating Data
		    	$query = DB::table('users_social_profile')
