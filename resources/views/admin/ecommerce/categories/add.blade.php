                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-content collpase show">
                            <div class="card-body">
                                <form class="form form-horizontal form-bordered" action="{{ route('insert_categories') }}" method="post" enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    <div class="form-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group row last">
                                                    <label class="label-control">Category Slug</label>
                                                    <input type="text" id="slug" name="slug" class="form-control" placeholder="Category Slug" value="{{ old('slug') }}">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group row">
                                                    <label class="label-control">Parent Categories</label>
                                                    <select id="parent_categories[]" name="parent_categories[]" class="form-control multi_select" multiple>
                                                        <option value="0" @if(old('parent_categories') == '0') selected @endif>No parent cateogry</option>
                                                        @if(!empty($parent_categories))
                                                            @foreach($parent_categories as $row)
                                                                <option value="{{ $row->id }}" @if(old('parent_categories') == $row->id) selected @endif>{{ $row->name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
